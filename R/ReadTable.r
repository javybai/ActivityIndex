...
{
  if (ncol(read.csv(file=filename,stringsAsFactors=FALSE,header=FALSE,nrows=1))!=3)
  {
    stop(paste0(filename," is not an appropriate 3-column data file!"))
  }
  result=fread(filename,sep=",",stringsAsFactors=FALSE,
               colClasses=rep("numeric",3),header=FALSE,
               showProgress=FALSE)
